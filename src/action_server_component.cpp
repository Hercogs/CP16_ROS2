...
// tortoisebot_waypoints_interfaces::action::WaypointAction; using
// GoalHandleWaypointAction =
//     rclcpp_action::ServerGoalHandle<WaypointAction>;

namespace mylib {

ActionServer::ActionServer() : Node("action_server_comp") {
  ...
  RCLCPP_INFO(this->get_logger(), "Action server, odom sub");

  pub_twist_ = this->create_publisher<Twist>("/cmd_vel", 5);
  RCLCPP_INFO(this->get_logger(), "Action server, cmd pub");
...
/* Odom callback*/
void ActionServer::odom_clb(const Odometry::SharedPtr msg) {
...
void ActionServer::execute(
  ...
  RCLCPP_INFO(this->get_logger(), "Executing goal: x: %.2f, y: %.2f, yaw: %.2f",
              goal->position.x, goal->position.y, goal->position.z);
